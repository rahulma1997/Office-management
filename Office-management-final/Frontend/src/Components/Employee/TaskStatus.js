...
export default function TaskStatus() {
  ...
  const fetchTaskStatuses = async () => {
    try {
      const response = await axios.get("http://localhost:5000/task-status");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingTaskId) {
        await axios.put(`http://localhost:5000/task-status/${editingTaskId}`, newTaskStatus);
      } else {
        await axios.post("http://localhost:5000/task-status", newTaskStatus);
      }
      fetchTaskStatuses();
      handleClose();
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this task status?")) {
      try {
        await axios.delete(`http://localhost:5000/task-status/${id}`);
        fetchTaskStatuses();
      } catch (error) {
