...
const ProjectReports = () => {
  ...
  const fetchProjects = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/projectreports');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await axios.put(`http://localhost:5000/api/projectreports/${editing}`, formData);
      } else {
        await axios.post('http://localhost:5000/api/projectreports', formData);
      }
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/api/projectreports/${id}`);
      fetchProjects();
    } catch (error) {
