...
const TaskManagement = () => {
    ...
    const fetchTasks = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/tasks/manager/get"
        );
    ...
    if (selectedTaskId) {
      ...
      try {
        const response = await axios.put(
          `http://localhost:5000/tasks/manager/${selectedTaskId}`,
          taskData
        );
        setTasks(
          tasks.map((task) =>
      ...
      try {
        const response = await axios.post(
          "http://localhost:5000/tasks/manager/create",
          taskData
        );
  ...
  const handleDeleteTask = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/tasks/manager/${id}`);
  ...
  const handleAssignTask = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/tasks/manager/Assign",
        taskData
      );
